...
The same wire story often runs on multiple outlets with near-identical headlines (e.g. Hindu BusinessLine and ET Tech both running "OpenAI's new device will be hockey puck-sized..." from the same source report). `dedupeItems()` in `scripts/fetch-feeds.mjs` detects these: items from *different* sources published within 48 hours of each other, whose titles share ≥35% of their significant words (Jaccard similarity on a stopword-filtered word set), get merged into one card — the newest becomes primary, the rest are listed as `relatedSources` and shown as a small "Also covered by ..." line plus a "🔥 Trending" badge instead of a separate duplicate card. The threshold is deliberately conservative (tuned against real data, not guessed) to avoid merging two different stories that just happen to share common AI vocabulary; expect it to only catch clear wire-copy duplicates, not independently-written retellings of the same news.
